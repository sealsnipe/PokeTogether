#!/usr/bin/env python
"""
Test für die verbesserte Multiplayer-Synchronisierung mit Fensterüberprüfung
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('test_multiplayer_sync_window.log')
    ]
)
...
def check_windows():
    """Überprüft, ob die Spielfenster angezeigt werden"""
    windows = gw.getAllTitles()
    logger.info(f"Gefundene Fenster: {windows}")
    
    player1_window = [w for w in windows if "Player 1" in w]
    player2_window = [w for w in windows if "Player 2" in w]
    
    if player1_window:
        logger.info(f"Player 1 Fenster gefunden: {player1_window[0]}")
    else:
        logger.warning("Player 1 Fenster nicht gefunden!")
    
    if player2_window:
        logger.info(f"Player 2 Fenster gefunden: {player2_window[0]}")
    else:
        logger.warning("Player 2 Fenster nicht gefunden!")
    
    return bool(player1_window) and bool(player2_window)

def main():
    """Hauptfunktion"""
    logger.info("=== STARTE FENSTER-TEST FÜR VERBESSERTE MULTIPLAYER-SYNCHRONISIERUNG ===")
    ...
    # Überprüfe, ob die Spielfenster angezeigt werden
    logger.info("Überprüfe, ob die Spielfenster angezeigt werden...")
    windows_found = check_windows()
    
    if not windows_found:
        logger.warning("Nicht alle Spielfenster wurden gefunden. Warte 5 Sekunden und versuche es erneut...")
        time.sleep(5)
        windows_found = check_windows()
    
    if windows_found:
        logger.info("Alle Spielfenster wurden gefunden!")
    else:
        logger.error("Nicht alle Spielfenster wurden gefunden!")
